...
import HomeViewStandingsListCon from "./HomeViewStandingsListCon";

const HomeViewStandingsCon = ({ constructorData, setSelectedConstructor, updateOrder}) => {
  const [isAscending, setIsAscending] = useState(true);

  const togglePosOrder = () => {
    updateOrder([...constructorData].reverse());
    ...
    <div className=" border-t-8 border-f1-black flex-col ">
      ...
      <div className="flex text-center border-b-2 border-f1-black pt-4 pr-4 font-f1 font-bold">
        <button
          ...
          className="w-1/5 text-left hover:text-candy-apple"
        >
          POS {isAscending ? "▲" : "▼"}
        </button>
        <h1 className="w-3/5"></h1>
        <h1 className="w-1/5">PTS</h1>
        <h1 className="w-1/5">WINS</h1>
      </div>
      <HomeViewStandingsListCon
...
export default HomeViewStandingsCon;
